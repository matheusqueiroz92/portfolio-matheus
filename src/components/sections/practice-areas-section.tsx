  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <FadeInStagger className="grid md:grid-cols-2 gap-6" stagger={0.15}>
          {services.map((service) => {
            const isFeatured = service.featured === true;
            return (
              <FadeInItem
                key={service.title}
                className={`group relative p-6 backdrop-blur-sm rounded-2xl border transition-all duration-500 hover:-translate-y-2 hover:border-primary/50 hover:shadow-[0_8px_30px_rgba(59,130,246,0.15),0_0_15px_rgba(59,130,246,0.1)] ${
                  isFeatured
              ...
              >
                {isFeatured && (
                  <span className="absolute right-5 top-5 inline-flex items-center gap-1 rounded-full border border-primary/40 bg-[color-mix(in_srgb,var(--primary)_12%,transparent)] px-3 py-1 text-[0.7rem] font-semibold uppercase tracking-wider text-primary">
